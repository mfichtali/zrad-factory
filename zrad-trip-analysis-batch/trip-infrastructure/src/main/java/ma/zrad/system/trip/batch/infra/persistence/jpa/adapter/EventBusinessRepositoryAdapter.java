...
import ma.zrad.system.trip.batch.infra.persistence.jpa.repository.EventBusinessJpaRepository;
...
public class EventBusinessRepositoryAdapter implements EventBusinessRepositoryPort {
    ...
    public List<EventBusinessDomain> findPendingEventByRegionAndSectionOfBatch(final EventBatchContextEnum batchCtx, final String regionCode, final String sectionCode) {
        var eventEntities = eventBusinessJpaRepository.findPendingEventByRegionAndSectionOfBatch(batchCtx, regionCode, sectionCode);
        return mapper.eventBusinessToDomains(eventEntities);
    }
    ...
    public List<EventRemainingCounterProjection> findRemainingPendingVehicle(final List<UUID> eventZrad01Ids, final String regionCode) {
        return eventBusinessJpaRepository.findRemainingPendingVehicle(eventZrad01Ids, regionCode);
    }
